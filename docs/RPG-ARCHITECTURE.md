...
## 화면 채움 (v5.5)
첫 화면은 로비 전신 일러스트 화풍의 새 키 비주얼을 사용합니다. viewport.css가 마지막에 적용되어 PC와 모바일에서 창 전체를 채웁니다. 모바일 브라우저 높이는 dvh로 추적하고 배경은 cover로 표시합니다. 전투 캔버스는 창 크기로 확장되며 입력 좌표도 fill 배율로 변환합니다. 16:9와 다른 비율에서는 전투 그림의 가로·세로 배율이 달라질 수 있습니다. 모바일 가로 잠금 요청 및 거부 시 회전 안내는 유지됩니다.
