...
export class Game {
 ...
 bindInput(){
  ...
  this.listen(this.canvas,'pointermove',e=>{const r=this.canvas.getBoundingClientRect();this.mouse=canvasPoint(r,e.clientX,e.clientY);});
  this.listen(this.canvas,'pointerdown',e=>{if(this.state!=='battle'||$('modal').open||this.victoryDelay>0||this.rpg?.transitioning)return;const r=this.canvas.getBoundingClientRect();this.mouse=canvasPoint(r,e.clientX,e.clientY);this.fx.emit('click',this.mouse.x,this.mouse.y,{size:65,amount:3});if(e.pointerType==='touch')return;if(e.button===0){this.firing=true;this.canvas.setPointerCapture(e.pointerId);this.attack(true);}else if(e.button===2){e.preventDefault();this.heavyAttack(true);}});
