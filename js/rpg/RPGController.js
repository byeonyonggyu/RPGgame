...
export class RPGController {
 ...
 clearRoom(){const g=this.game;g.input?.reset();g.keys?.clear();g.firing=false;for(const key of ['enemies','bullets','hazards','effects','drops','nodes'])g[key]=[];g.boss=null;g.fx?.clear();this.campVisited=false;this.exhibits?.assembly.clear();}
 ...
 draw(c){if(this.active&&this.area.current)this.exhibits.room(c,this.area.current,this.data,this.game.time,p=>this.restorationProgress(p));}
}
